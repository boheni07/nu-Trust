...
CURRENT_TAG=$(docker inspect --format='{{index .Config.Image}}' "$(docker compose -f docker-compose.staging.yml ps -q nutrust-backend 2>/dev/null | head -1)" 2>/dev/null | awk -F: '{print $NF}' || echo "")
